...
const AdminHome = () => {
  ...
  const [nosoldart, setNosoldart] = useState([]);
    ...
    const getCount = async () => {
      try {
        const res = await Axios.get(
          `http://localhost:5000/api/artworks/getcountUsers`,
          {
            headers: {
  ...
  useEffect(() => {
    const getSoldpainting = async () => {
      try {
        const res = await Axios.get(
          `http://localhost:5000/api/artworks/getnosoldart`,
          {
            headers: {
              Authorization: "Bearer " + authCtx.token,
            },
    ...
    <>
      ...
      <div style={{ padding: "5%", display: "flex", marginLeft: "150px" }}>
        <DCard>
          ...
          <Number>{countuser.usercount}</Number>
        </DCard>
        <DCard>
          <Name>Painting Sold </Name>
          <Number>{nosoldart.artcount}</Number>
        </DCard>
      </div>
