...
const Artwork = () => {
  ...
  const handleEditArtwork = (id, instock) => {
    ...
    Axios.put(`http://localhost:5000/api/artworks//update/${id}`, {
      instock: instock
    }, {
      headers: {
        Authorization: "Bearer " + authCtx.token,
      ...
      return (
        <tbody>
          <tr>
            ...
            <td className="admintd"><Link to={`/EditArtwork/${item.id}`} style={{ textDecoration: "none", color: "black" }}><EditIcon /></Link></td>
          </tr>
        </tbody>
      );
    });
